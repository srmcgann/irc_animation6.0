(() => {
  spawnCol = () => {
    ...
    do{
      ...
      for(j = 0; !occupied && j < cols[i].length; ++j) {
        if(cols[i][j].id !== null) occupied = true
      }
      checks++
    } while (occupied && checks < 2);
    if (!occupied) {
      cols[i][0] = {id: R() * 4 | 0, alpha: 1.15, life: 0}
    }
  }
    ...
    for(let i = 0; i < cols.length; ++i) {
      ...
      for( let j = cols[i].length - 1; j--; ) {
        if (cols[i][j].id !== null) {
          ...
          if (bottomChar === -1 && j < cols[i].length - 2) {
            bottomChar = j
            cols[i][j].life++
  ...
  let colNo = c.width / (size * spacing) | 0
  let cols = new Array(colNo).fill().map(e=>e=new Array(rowNo).fill().map(e=>e={id: null, alpha: 0, life: 0}))
  window.addEventListener('resize', e => {
    ...
    colNo = c.width / (size * spacing) | 0
    cols = new Array(colNo).fill().map(e=>e=new Array(rowNo).fill().map(e=>e={id: null, alpha: 0, life: 0}))
  })
  let bg = new Image()
  bg.onload = e => {
    bg.loaded = true
  ...
  //logo[1].img.src = 'i.png'
  //logo[2].img.src = 'r.png'
  //logo[3].img.src = 'c.png'
})()